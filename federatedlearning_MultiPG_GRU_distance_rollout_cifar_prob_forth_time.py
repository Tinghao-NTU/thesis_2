...
from utils.update_device_LSTM_prob_forth import local_update,get_state,permute_device
...
name1 = 'TrainingRecords_'+str(each_num*10)+bias+'_test'
name2 = 'Device_list_prob'+bias+'_test'
name3 = 'Rewards_prob'+'_test'
...
#     torch.save(net_glob_init.state_dict(),'test_cifar.pkl')
    ...
    for roll_num in range(30):
        ...
        while not done: 
            ...
            idxs_users = torch.multinomial(torch.from_numpy(pk), 10, replacement=False).tolist()
            end = time.perf_counter()
            E,T,_, p_list, b_list, g_list = optimization_bf(f_list, p, g, C, D, 10000, s, B,idxs_users)
            
            
            file_handle=open('ET'+name1+'prob_cifar_'+str(roll_num)+'.txt',mode='a')
