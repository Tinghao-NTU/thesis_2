...
if bias == 'noniid':
    ...
    with open('device_index_mnist_iid.json', 'r') as f:
        clusteded_dic = json.load(f)
...
name1 = 'TrainingRecords_fedavg_'+str(num_devices)+bias+'_'
name2 = 'Device_list_fedavg_'+bias
name3 = 'Rewards_fedavg_'+bias
...
for ep_iter in range(10000):
    ...
    while done != 1:
        ...
        next_state_matrix,net_glob,acc_list,cluster_acc,w_list, reward_n,done_n= local_update(args,dataset_train,dataset_test,dict_users,
                 idxs_users, epoch,net_glob,state_matrix = state_matrix, acc_list = acc_list, cluster_acc = cluster_acc, w_list = w_list, name =name1+'mnist_'+str(ep_iter)+'.txt',preset  =preset_accuracy,count_list = count_list)
