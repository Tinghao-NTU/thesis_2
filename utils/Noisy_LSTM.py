...
class NoisyDQN(nn.Module):
    def __init__(self, num_inputs, num_actions):
        ...
        self.noisy2 = NoisyLinear(256, 1)
        
    def forward(self, x):
        hns = torch.zeros(2,x.size()[0],self.n_hidden).cuda()
        cns = torch.zeros(2,x.size()[0],self.n_hidden).cuda()
        outputs = []
 
        for i in range(100):
            input = x.view(-1,100,50)[:,i,:].view(-1,1,50).float()
            output, (hns, cns) = self.LSTM(input, (hns, cns))
            output_policy = self.noisy2(self.noisy1(output))
