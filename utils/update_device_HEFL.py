...
def feature_cal_all(wlist, wglobal):
    ...
    for i in range(100):
        ...
        state_list.append(torch.norm(torch.cat(para_list), p = 1,dim = 0).item())
#     para_list = torch.cat(para_list)
...
def local_update(args,dataset_train,dataset_test,dict_users,idxs_users,epoch,net_glob, acc_list = [],cluster_acc = None, w_list = None, name = None,preset  = None,count_list = None):
    w_locals_temp = []
    loss_locals = []
    ...
    reward_n,f_list, p_list, b_list, g_list = optimization_bf(f, p, g, C, D, k, s, B,idxs_users)
    reward_n = -reward_n
    if preset is None:
        preset  = 99
...
#             cluster_acc.append([acc_train.numpy().item()])
    ...
    state_matrix = feature_cal_all(w_list, w_glob)*g*1e12 / D
...
#     state_matrix = np.concatenate([state_matrix,count_list,np.array(acc_train-preset)])
    state_matrix = np.concatenate([state_matrix,f_list/1e9,p_list,b_list/1e6,np.array(acc_train-preset)])
