...
if bias == 'noniid':
    ...
    checkpoint_DRL = torch.load('ppo_LSTM_cifar_noniid.pkl', map_location='cpu')
    with open('data_05.json', 'r') as f:
        dict_users = json.load(f)
else:
    preset_accuracy = torch.tensor([56.5])
    checkpoint_DRL = torch.load('ppo_LSTM_cifar_iid.pkl', map_location='cpu')
...
class ActorCritic(nn.Module):
    def __init__(self, num_inputs, num_outputs, hidden_size, std=0.0):
        ...
        self.gru_critic = nn.GRU(num_inputs, hidden_size, 2, batch_first = True)
        ...
        self.gru_actor = nn.GRU(num_inputs, hidden_size, 2, batch_first = True)
...
model = ActorCritic(5, 100, 256).to(device)
...
k = 1
name1 = 'TrainingRecords_LSTM_'+str(num_devices)+bias+'_'+str(k)+'_test_'
name2 = 'Device_list_ppo_LSTM_'+bias+'_'+str(k)+'_test_'
name3 = 'Rewards_ppo_LSTM_'+bias+'_'+str(k)+'_test_'
...
for ep_iter in range(30):
    ...
    while done != 1:
        ...
        file_handle=open('Time'+name1+'PPO_cifar_'+'.txt',mode='a')
...
#         print(np.sort(idxs_users))
        ...
        next_state_matrix,net_glob,acc_list,cluster_acc,w_list, reward_n,done_n= local_update(args,dataset_train,dataset_test,dict_users,
                    idxs_users, epoch,net_glob, acc_list = acc_list, cluster_acc = cluster_acc, w_list = w_list, name =name1+'ppo_cifar_'+str(ep_iter)+'.txt',preset  =preset_accuracy,count_list = count_list,k = k)
