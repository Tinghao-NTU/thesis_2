...
#         value = self.critic2(input_critic)
        return input_critic.mean(1).unsqueeze(1)
...
name1 = 'TrainingRecords_LSTM_forth'+str(num_devices)+bias+'_'
name2 = 'Device_list_ppo_LSTM_forth'+bias
name3 = 'Rewards_ppo_LSTM_forth'+bias
...
#         print(np.sort(idxs_users))
    ...
    if ep_iter > 20:
        if np.mean(all_rewards[-20:]) > max_reward:
            torch.save(model_actor.state_dict(), 'ppo_LSTM_cifar_actor_forth'+bias+'.pkl')
            torch.save(model_critic.state_dict(), 'ppo_LSTM_cifar_critic_forth'+bias+'.pkl')
