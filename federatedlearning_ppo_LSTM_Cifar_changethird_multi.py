...
from utils.update_device_dis_test import local_update,get_state,permute_device,feature_cal_all
...
if bias == 'noniid':
    ...
    checkpoint_DRL = torch.load('ppo_LSTM_cifar_actornoniid.pkl', map_location='cpu')
    with open('data_05.json', 'r') as f:
        dict_users = json.load(f)
else:
    checkpoint_DRL = torch.load('ppo_LSTM_cifar_actoriid.pkl', map_location='cpu')
...
num_devices_list = [100, 70, 50, 30, 10]
log_probs = []
values    = []
states    = []
actions   = []
...
for num_devices in num_devices_list:
    name1 = 'TrainingRecords_LSTM_changethird_test_multi_'+str(num_devices)+bias+'_'
    name2 = 'Device_list_ppo_LSTM_changethird_test_multi_'+str(num_devices)+bias+'_'
    name3 = 'Rewards_ppo_LSTM_changethird_test_multi_'+str(num_devices)+bias+'_'

    update_counts = 0
    for ep_iter in range(10):
        w_list = []
        losses = []
        ...
        while done != 1:
            ...
            idxs_users = pmt[torch.multinomial(F.softmax(action,dim = 1), num_devices, replacement=False).detach().cpu().view(-1)]
    ...
    #         print(np.sort(idxs_users))
            ...
            next_state_matrix,net_glob,acc_list,cluster_acc,w_list, reward_n,done_n= local_update(args,dataset_train,dataset_test,dict_users,
                        idxs_users, epoch,net_glob, k = 1, acc_list = acc_list, cluster_acc = cluster_acc, w_list = w_list, name =name1+'ppo_cifar_'+str(ep_iter)+'.txt',preset  =preset_accuracy,count_list = count_list)
