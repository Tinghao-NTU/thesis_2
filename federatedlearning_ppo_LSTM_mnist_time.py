...
if bias == 'noniid':
    ...
    checkpoint_DRL = torch.load('ppo_LSTM_mnist_noniid.pkl', map_location='cpu')
    with open('data_mnist_05.json', 'r') as f:
        dict_users = json.load(f)
else:
    preset_accuracy = torch.tensor([99.0])
    checkpoint_DRL = torch.load('ppo_LSTM_mnist_iid.pkl', map_location='cpu')
...
for ep_iter in range(10000):
    ...
    while done != 1:
        ...
        file_handle=open('Time'+name1+'PPO_mnist_'+'.txt',mode='a')
...
#         print(np.sort(idxs_users))
        ...
        next_state_matrix,net_glob,acc_list,cluster_acc,w_list, reward_n,done_n= local_update(args,dataset_train,dataset_test,dict_users,
                    idxs_users, epoch,net_glob, acc_list = acc_list, cluster_acc = cluster_acc, w_list = w_list, name =name1+'ppo_mnist_'+str(ep_iter)+'.txt',preset  =preset_accuracy,count_list = count_list,k=k)
