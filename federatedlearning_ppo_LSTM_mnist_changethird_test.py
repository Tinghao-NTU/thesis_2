...
if bias == 'noniid':
    ...
    checkpoint_DRL = torch.load('ppo_LSTM_mnist_actornoniid.pkl', map_location='cpu')
...
#         print(np.sort(idxs_users))
        ...
        next_state_matrix,net_glob,acc_list,cluster_acc,w_list, reward_n,done_n= local_update(args,dataset_train,dataset_test,dict_users,
                    idxs_users, epoch,net_glob,k = 1, acc_list = acc_list, cluster_acc = cluster_acc, w_list = w_list, name =name1+'ppo_mnist_'+str(ep_iter)+'.txt',preset  =preset_accuracy,count_list = count_list)
