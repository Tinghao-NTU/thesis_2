...
name3 = 'Rewards_ppo_LSTM_changethird_flat_'+bias
...
#         print(np.sort(idxs_users))
    ...
    if ep_iter > 20:
        if np.mean(all_rewards[-20:]) > max_reward:
            torch.save(model_actor.state_dict(), 'ppo_LSTM_mnist_actor_flat_'+bias+'.pkl')
            torch.save(model_critic.state_dict(), 'ppo_LSTM_mnist_critic_flat_'+bias+'.pkl')
