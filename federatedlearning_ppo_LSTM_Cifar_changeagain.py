...
from utils.update_device_dis_GRU import local_update,get_state,permute_device,feature_cal_all
...
class Actor(nn.Module):
    def __init__(self, num_inputs, num_outputs, hidden_size, std=0.0):
        ...
        self.gru = nn.GRU(num_inputs, hidden_size, 1, batch_first = True)
    ...
    def forward(self, x):
        ...
        input_all,_ = self.gru(x)
        mu    = self.actor(input_all).view(batch_size,seq_len)
...
class Critic(nn.Module):
    def __init__(self, num_inputs, num_outputs, hidden_size, std=0.0):
        ...
        self.hidden_size = hidden_size
        self.gru = nn.GRU(num_inputs, hidden_size, 1, batch_first = True)
        self.critic1 = nn.Linear(hidden_size * 100, hidden_size)
    ...
    def forward(self, x):
        batch_size, seq_len, input, = x.size()
        input_all,_ = self.gru(x)
        input_critic = input_all.reshape(batch_size,-1)
        input_critic = F.relu(self.critic1(input_critic))
...
name1 = 'TrainingRecords_LSTM_change_flat_'+str(num_devices)+bias+'_'
name2 = 'Device_list_ppo_LSTM_change_flat_'+bias
name3 = 'Rewards_ppo_LSTM_change_flat_'+bias
...
#         print(np.sort(idxs_users))
        ...
        next_state_matrix,net_glob,acc_list,cluster_acc,w_list, reward_n,done_n= local_update(args,dataset_train,dataset_test,dict_users,
                    idxs_users, epoch,net_glob, acc_list = acc_list, cluster_acc = cluster_acc, w_list = w_list, name =name1+'ppo_cifar_'+str(ep_iter)+'.txt',preset  =preset_accuracy,count_list = count_list)
        ...
        if update_counts % num_steps == 0:
            next_state = torch.FloatTensor(next_state_matrix).unsqueeze(0).to(device)
    ...
    if ep_iter > 20:
        if np.mean(all_rewards[-20:]) > max_reward:
            torch.save(model_actor.state_dict(), 'ppo_LSTM_cifar_actor_flat_'+bias+'.pkl')
            torch.save(model_critic.state_dict(), 'ppo_LSTM_cifar_critic_flat_'+bias+'.pkl')
