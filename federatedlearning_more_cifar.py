...
def add_device(idx_list,remain_index,T_sum):
    ...
    for i in remain_index:
        ...
        E,T,_, _, b_list, g_list = optimization_bf(f_list, p, g, C, D, 1, 4894444, B,temp_index)
#         print(b_list)
        if T < min_T:
            min_idx = i
            min_T = T
...
backup = [[34, 16],
[34, 16, 75, 99, 53],
[34, 16, 75, 99, 53, 97, 14, 43, 77, 15, 87, 82, 67, 69, 79, 61, 26, 47, 55, 60, 95, 46, 63, 64],
[34, 16, 75, 99, 53, 97, 14, 43, 77, 15, 87, 82, 67, 69, 79, 61, 26, 47, 55, 60, 95, 46, 63, 64, 92, 54, 30, 57, 59, 24, 80, 48, 76, 40, 49, 9, 65, 90, 58, 71, 94, 81, 41, 78, 22, 29],
[34, 16, 75, 99, 53, 97, 14, 43, 77, 15, 87, 82, 67, 69, 79, 61, 26, 47, 55, 60, 95, 46, 63, 64, 92, 54, 30, 57, 59, 24, 80, 48, 76, 40, 49, 9, 65, 90, 58, 71, 94, 81, 41, 78, 22, 29, 96, 35, 32, 25, 11, 56, 62, 2, 37, 44, 51, 6, 45, 42, 84, 28, 73, 5, 50, 4]
]
...
#             print(np.mean(delta))
            ...
            #file_handle=open('Time'+name1+'more_cifar_'+str(roll_num)+'.txt',mode='a')
            #file_handle.write(str(end-start))
            #file_handle.write('\n')
            #file_handle.close()

            file_handle=open('ET'+name1+'more_cifar_'+str(roll_num)+'.txt',mode='a')
            file_handle.write(str(E)+' ' + str(T))
            file_handle.write('\n')
            file_handle.close()
            
            #file_handle=open('Device'+name1+'more_cifar_'+str(roll_num)+'.txt',mode='a')
    ...
    dict_users,idxs_users,epoch,net_glob,rou= rou,beta=beta,delta = delta,acc_list = acc_list, w_list = w_list, name =name1+'cifar_'+str(roll_num)+'.txt',preset = preset_accuracy)            
