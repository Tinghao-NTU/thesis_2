...
from utils.update_device_dis_DNN import local_update,get_state,permute_device,feature_cal_all
...
name1 = 'TrainingRecords_DNN_'+str(num_devices)+bias+'_'
name2 = 'Device_list_ppo_DNN_'+bias
name3 = 'Rewards_ppo_DNN_'+bias
...
#         print(np.sort(idxs_users))
    ...
    if ep_iter > 20:
        if np.mean(all_rewards[-20:]) > max_reward:
            torch.save(model.state_dict(), 'ppo_DNN_cifar_'+bias+'.pkl')
