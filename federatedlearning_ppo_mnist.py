...
for ep_iter in range(500):
    ...
    while done != 1:
        ...
        next_state_matrix,net_glob,acc_list,cluster_acc,w_list, reward_n,done_n= local_update(args,dataset_train,dataset_test,dict_users,
                 idxs_users, epoch,net_glob, acc_list = acc_list, cluster_acc = cluster_acc, w_list = w_list, name =name1+'mnist_'+str(ep_iter)+'.txt',preset  =preset_accuracy,count_list = count_list)
    ...
    if ep_iter > 20:
        if np.mean(all_rewards[-20:]) > max_reward:
            torch.save(model.state_dict(), 'ppo_mnist_'+bias+'.pkl')
