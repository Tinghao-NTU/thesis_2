...
scaler_2 = MinMaxScaler(feature_range=(-5, 5)) 
...
#     para_list = torch.cat(para_list)
    return torch.sum(torch.cat(state_list,0),1)
...
#         state = np.expand_dims(state,0)
    ...
    return scaler_2.fit_transform(state[device_idx].reshape(-1,1)).reshape(1,-1)
