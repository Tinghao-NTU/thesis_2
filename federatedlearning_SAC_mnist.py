...
for ep_iter in range(10000):
    ...
    while done != 1:
        ...
        file = open(name2+'_mnist_'+str(ep_iter)+'.txt',mode='a')
        ...
        next_state_matrix,net_glob,acc_list,cluster_acc,w_list, reward_n,done_n= local_update(args,dataset_train,dataset_test,dict_users,
                 idxs_users, epoch,net_glob,state_matrix = state_matrix,acc_list = acc_list, cluster_acc = cluster_acc, w_list = w_list, name =name1+'sac_mnist_'+str(ep_iter)+'.txt',preset  =preset_accuracy,count_list = count_list)
    ...
    if len(replay_buffer) > 1024:
        if np.mean(all_rewards[-5:]) > np.mean(all_rewards[-6:-1]):
            torch.save(policy_net.state_dict(), 'SAC_mnist_'+bias+'.pkl')