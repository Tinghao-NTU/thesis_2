...
name1 = 'TrainingRecords_LSTM_changethird'+str(num_devices)+bias+'_'
name2 = 'Device_list_ppo_LSTM_changethird'+bias
name3 = 'Rewards_ppo_LSTM_changethird'+bias
...
#         print(np.sort(idxs_users))
    ...
    if ep_iter > 20:
        if np.mean(all_rewards[-20:]) > max_reward:
            torch.save(model_actor.state_dict(), 'ppo_LSTM_cifar_actor'+bias+'.pkl')
            torch.save(model_critic.state_dict(), 'ppo_LSTM_cifar_critic'+bias+'.pkl')
