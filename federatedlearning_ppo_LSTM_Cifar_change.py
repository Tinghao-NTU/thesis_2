...
class ActorCritic(nn.Module):
    ...
    def forward(self, x):
        ...
        input_critic = F.relu(self.critic1(input_all).view(batch_size,-1))
...
name1 = 'TrainingRecords_LSTM_change_'+str(num_devices)+bias+'_'
name2 = 'Device_list_ppo_LSTM_change_'+bias
name3 = 'Rewards_ppo_LSTM_change_'+bias
...
#         print(np.sort(idxs_users))
    ...
    if ep_iter > 20:
        if np.mean(all_rewards[-20:]) > max_reward:
            torch.save(model.state_dict(), 'ppo_LSTM_cifar_change_'+bias+'.pkl')
