...
from utils.update_device_dis_test_trans import local_update,get_state,permute_device
...
name1 = 'TrainingRecords_fedavg_trans_'+str(num_devices)+bias+'_test_'
name2 = 'Device_list_fedavg_trans_'+bias+'_test_'

name3 = 'Rewards_fedavg_trans_'+bias+'_test_'
...
for ep_iter in range(30):
    ...
    while done != 1:
        ...
        idxs_users = torch.multinomial(torch.Tensor(all_users), 10, replacement=False).tolist()
        ...
        next_state_matrix,net_glob,acc_list,cluster_acc,w_list, reward_n,done_n= local_update(args,dataset_train,dataset_test,dict_users,
                 idxs_users, epoch,net_glob, acc_list = acc_list, cluster_acc = cluster_acc, w_list = [w_glob_init]*100, name =name1+'mnist_'+str(ep_iter)+'.txt',preset  =preset_accuracy,count_list = [0]*100)
